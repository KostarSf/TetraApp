...
const TaskScreen: React.FC = () => {
  ...
  return (
    <div className="container-fluid h-100 ">
      ...
      <div>
        <Fakecrumb
          fakeItems={["Проекты", "ООО «Дельта Юг»"]}
          items={[{ title: "Доска задач компании", link: "/dashboard" }]}
          ...
          <div className="col-xl-8 col-lg-7 col-md-7">
            <div className="p-3">
              <h2>
                {task.title}{" "}
                {isNew && (
                  <span className="badge bg-primary ms-4 fs-5">Новое</span>
                )}
              </h2>
              <p>{task.description}</p>
              <div className="d-flex gap-4 flex-wrap text-secondary">
                Добавлено{" "}
                {RelativeTime.fromNowOn(task.creationDate).toLowerCase()}, в{" "}
                {RelativeTime.displayTime(task.creationDate)}
                <p className="position-relative">
                  <>
                    <span
                      className={`p-1 rounded-circle badge d-inline-block bg-${board.alterColor}`}
                    ></span>
              ...
              <div className="d-grid d-sm-flex gap-2">
                {DataActions.getAllBoardsWithSort()
                  .filter((b) => !b.newBoard)
                  .map((b) => {
                    return (
                      <ChangeBoardButton
            ...
            <hr />
            <div className="p-3 fs-5">{parser.toReact(task.legend)}</div>
            <hr />
            ...
            <div className="col">
              <div className="p-4 m-3 mt-4 border rounded bg-white">
                ...
                <div className="row mb-3">
                  ...
                  <div className="col">
                    {workTaskData &&
                      DataActions.getUserById(workTaskData.managerId)?.fullName}
                  </div>
                </div>
                <div className="row mb-3 flex-wrap">
                  ...
                  <div className="col">
                    <ul className="list-group list-group-flush">
                      ...
                      {workTaskData &&
                        DataActions.getAllUsers()
                          .filter((u) =>
                            workTaskData.memberIds.find((m) => m === u.id)
                          )
                          .map((u) => <li key={u.id}>{u.fullName}</li>)}
                    </ul>
                  </div>
                </div>
